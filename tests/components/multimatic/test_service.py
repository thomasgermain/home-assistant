...
from homeassistant.components import multimatic
from homeassistant.components.multimatic import DOMAIN
...
async def test_remove_quick_mode(hass):
    ...
    await call_service(hass, "multimatic", "remove_quick_mode", None)
...
async def test_remove_quick_mode_wrong_data(hass):
    ...
    with pytest.raises(voluptuous.error.MultipleInvalid):
        await call_service(hass, "multimatic", "remove_quick_mode", {"test": "boom"})
...
async def test_remove_holiday_mode(hass):
    ...
    assert await setup_multimatic(hass, system=system)
    await call_service(hass, "multimatic", "remove_holiday_mode", None)
...
async def test_remove_holiday_mode_wrong_data(hass):
    ...
    assert await setup_multimatic(hass)
    with pytest.raises(voluptuous.error.MultipleInvalid):
        await call_service(hass, "multimatic", "remove_holiday_mode", {"test": "boom"})


async def test_set_quick_mode(hass):
    """Set quick mode."""
    assert await setup_multimatic(hass)
    await call_service(hass, "multimatic", "set_quick_mode", {"quick_mode": "QM_PARTY"})
...
async def test_set_quick_mode_wrong_data(hass):
    ...
    assert await setup_multimatic(hass)
    with pytest.raises(voluptuous.error.MultipleInvalid):
        await call_service(hass, "multimatic", "set_quick_mode", {"test": "boom"})
...
async def test_set_holiday_mode_wrong_data(hass):
    ...
    assert await setup_multimatic(hass)
    with pytest.raises(voluptuous.error.MultipleInvalid):
        await call_service(hass, "multimatic", "set_holiday_mode", {"test": "boom"})
...
async def test_remove_quick_veto_wrong_data(hass):
    ...
    assert await setup_multimatic(hass)
    await call_service(
        hass, "multimatic", "remove_quick_veto", {"entity_id": "climate.test123"}
    )
...
async def test_remove_quick_veto_room(hass):
    ...
    assert await setup_multimatic(hass, system=system)
    await call_service(
        hass, "multimatic", "remove_quick_veto", {"entity_id": "climate.room_1"}
    )
...
async def test_no_remove_quick_veto_zone(hass):
    ...
    assert await setup_multimatic(hass)
    await call_service(
        hass, "multimatic", "remove_quick_veto", {"entity_id": "climate.zone_1"}
    )
    SystemManagerMock.instance.remove_zone_quick_veto.assert_not_called()
...
async def test_set_quick_veto_room(hass):
    ...
    await call_service(
        ...
        DOMAIN,
        "set_quick_veto",
        {"entity_id": "climate.room_1", "duration": 300, "temperature": 25},
    )
...
async def test_set_quick_veto_zone_with_quick_veto(hass):
    ...
    await call_service(
        ...
        DOMAIN,
        "set_quick_veto",
        {"entity_id": "climate.zone_1", "duration": 300, "temperature": 25},
    )
