...
from homeassistant.components.multimatic.const import ATTR_MULTIMATIC_MODE
...
def _assert_state(hass, mode, temp, current_temp, away_mode):
    ...
    state = hass.states.get("water_heater.dhw")
    assert hass.states.is_state("water_heater.dhw", mode.name)
...
def fixture_only_water_heater(mock_system_manager):
    ...
    multimatic.PLATFORMS = ["water_heater"]
    yield
...
async def test_turn_away_mode_on(hass):
    ...
    await hass.services.async_call(
        "water_heater",
        "set_away_mode",
        {"entity_id": "water_heater.dhw", "away_mode": True},
    )
...
async def test_turn_away_mode_off(hass):
    ...
    await hass.services.async_call(
        "water_heater",
        "set_away_mode",
        {"entity_id": "water_heater.dhw", "away_mode": False},
    )
    await hass.async_block_till_done()
...
async def test_set_operating_mode(hass):
    ...
    await hass.services.async_call(
        "water_heater",
        "set_operation_mode",
        {"entity_id": "water_heater.dhw", "operation_mode": "ON"},
    )
    await hass.async_block_till_done()
...
async def test_set_operating_mode_wrong(hass):
    ...
    await hass.services.async_call(
        "water_heater",
        "set_operation_mode",
        {"entity_id": "water_heater.dhw", "operation_mode": "wrong"},
    )
    await hass.async_block_till_done()
...
async def test_set_temperature(hass):
    ...
    await call_service(
        ...
        "water_heater",
        "set_temperature",
        {"entity_id": "water_heater.dhw", "temperature": 50},
    )
...
async def test_set_operating_mode_while_quick_mode(hass):
    ...
    await call_service(
        ...
        "water_heater",
        "set_operation_mode",
        {"entity_id": "water_heater.dhw", "operation_mode": "AUTO"},
    )
    SystemManagerMock.instance.set_hot_water_operating_mode.assert_called_once_with(
